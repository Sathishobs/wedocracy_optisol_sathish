...
// general appearance
static const CGFloat CSToastMaxWidth            = 0.95;      // 95% of parent view width
...
- (void)makeToast:(NSString *)message errorType:(CSNotificationViewStyle)notificationType{
    [self makeToast:message duration:CSToastDefaultDuration position:CSToastDefaultPosition errorType:notificationType];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position errorType:(CSNotificationViewStyle)notificationType{
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:duration position:position errorType:notificationType];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title errorType:(CSNotificationViewStyle)notificationType{
    UIView *toast = [self viewForMessage:message title:title image:nil];
    [self showToast:toast duration:duration position:position errorType:notificationType];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position image:(UIImage *)image errorType:(CSNotificationViewStyle)notificationType{
    message = [message copy];
    UIView *toast = [self viewForMessage:message title:nil image:image];
    [self showToast:toast duration:duration position:position errorType:notificationType];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(id)position title:(NSString *)title image:(UIImage *)image errorType:(CSNotificationViewStyle)notificationType{
    UIView *toast = [self viewForMessage:message title:title image:image];
    [self showToast:toast duration:duration position:position errorType:notificationType];
}

- (void)showToast:(UIView *)toast errorType:(CSNotificationViewStyle)notificationType{
    [self showToast:toast duration:CSToastDefaultDuration position:CSToastDefaultPosition errorType:notificationType];
}
...
- (UIColor*)blurTintColorForStyle:(CSNotificationViewStyle)style
{
    ...
    switch (style) {
        case CSNotificationViewStyleSuccess:
            blurTintColor = [UIColor colorWithRed:0.00 green:0.50 blue:0.00 alpha:1.0];
            break;
        case CSNotificationViewStyleError:
            blurTintColor = [UIColor colorWithRed:0.72 green:0.00 blue:0.00 alpha:1.0];
            break;
        default:
            break;
    }
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point errorType:(CSNotificationViewStyle)notificationType {
    
    toast.center = [self centerPointForPosition:point withToast:toast];
    toast.alpha = 0.0;
    toast.backgroundColor = [self blurTintColorForStyle:notificationType];
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if(image != nil) {
        ...
        imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"v_logo.png"] ];
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        imageView.frame = CGRectMake(CSToastHorizontalPadding, CSToastVerticalPadding, CSToastImageViewWidth, CSToastImageViewHeight);
    }
